...
let selectedId = null;
let doneOpen = false;
let overdueHiddenOpen = false;                     // 숨긴 밀린 할 일 섹션 접힘(기본 닫힘)
...
function buildIndex() {
  return {
    ...
    todos: data.todos.map(t => ({
      ...
      hidden: t.hidden ? true : undefined,                    // 밀린 할 일 숨김
    })),
  };
}
...
// 현재 뷰의 "밀린 할 일" 경계 (그 이전 날짜가 밀린 항목). 현재 기간에서만 유효.
function overdueBoundary(v) {
  const tk = todayKey();
  if (v.type === 'today') return tk;
  if (v.type === 'week' && mondayKeyOf(v.anchor || tk) === mondayKeyOf(tk)) return mondayKeyOf(tk);
  if (v.type === 'month' && monthKeyOf(v.anchor || tk) === monthKeyOf(tk)) return monthKeyOf(tk) + '-01';
  return null;
}
// 현재 뷰에서 이 항목이 '밀린 할 일'인가
function isOverdueItem(t) {
  const b = overdueBoundary(view);
  return !!(b && isTodo(t) && !t.done && t.date !== null && t.date < b);
}
// 이 항목의 고정 범위: 밀린 항목이면 안정적인 'overdue'(날짜 바뀌어도 유지), 아니면 뷰별 범위
function pinScopeFor(t) { return isOverdueItem(t) ? 'overdue' : viewScope(); }

function togglePin(id) {
  const t = data.todos.find(x => x.id === id);
  if (!t) return;
  const scope = pinScopeFor(t);
  if (!scope) return;
  flushPendingEdits();
...
function renderList() {
  ...
  if (view.type === 'done') {
    ...
    // 상단 고정 (밀린 항목은 'overdue' 범위, 나머지는 뷰 범위 — 고정은 맨 위로)
    const scope = viewScope();
    let pinned = [];
    if (scope) {
      const isP = t => isPinned(t, pinScopeFor(t));
    ...
    // 밀린 할 일 — 보이는 것 / 숨긴 것 분리 (숨김은 해제 전까지 유지)
    const overdueVisible = overdue.filter(t => !t.hidden);
    const overdueHidden = overdue.filter(t => t.hidden);
    if (overdueVisible.length > 0) {
      addSec(`밀린 할 일 ${overdueVisible.length}`, 'overdue');
      addRows(overdueVisible.sort(byDate));
    }
    if (overdueHidden.length > 0) {
      const hbtn = document.createElement('button');
      hbtn.type = 'button';
      hbtn.className = 'done-toggle';
      hbtn.textContent = `${overdueHiddenOpen ? '▾' : '▸'} 🙈 숨긴 밀린 할 일 ${overdueHidden.length}`;
      hbtn.addEventListener('click', () => { overdueHiddenOpen = !overdueHiddenOpen; renderList(); });
      frag.appendChild(hbtn);
      if (overdueHiddenOpen) addRows(overdueHidden.sort(byDate));
    }
...
function rowEl(t) {
  ...
  const scope = viewScope();
  const overdueItem = isOverdueItem(t);
  // 자료 뷰에선 자료가, 그 외(할일/그룹 뷰)에선 할일이 고정 대상 (회의는 고정 안 함)
  const pinnable = !!scope && (view.type === 'notes' ? note : isTodo(t));
  // 밀린 항목은 'overdue' 범위로 고정(날짜 바뀌어도 유지)
  const pinned = pinnable && isPinned(t, pinScopeFor(t));
  ...
  row.innerHTML =
    ...
    (overdueItem ? `<button class="row-hide${t.hidden ? ' on' : ''}" type="button" tabindex="-1" title="${t.hidden ? '다시 표시' : '숨기기'}">${t.hidden ? '👁' : '🙈'}</button>` : '') +
...
function bindEvents() {
  ...
  $('todoList').addEventListener('click', e => {
    ...
    if (e.target.closest('.row-hide')) { toggleHidden(row.dataset.id); return; }
    if (e.target.closest('.todo-check')) { toggleDone(row.dataset.id); return; }
